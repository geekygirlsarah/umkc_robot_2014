...
//	string filename(argv[1]);
	...
	Mat match_frame;
	match_frame = imread("./flame_compare_outline.png", CV_LOAD_IMAGE_GRAYSCALE);
	...
	//	drawContours(frame, contours, maxPosition.y, CV_RGB(0xFF, 0x66, 0x99), CV_FILLED, CV_AA, hierarchy, INT_MAX, Point(0, 150));
		...
		drawContours(frame, contours, -1, CV_RGB(0xFF, 0x66, 0x99), CV_FILLED, CV_AA, hierarchy, INT_MAX, Point(0, 150));
		cvtColor(thresh, thresh, CV_GRAY2RGB);
	//	hconcat(frame, thresh, window);

		imshow("image raw", viewport);
	//	imshow("image thresh", match_frame);
