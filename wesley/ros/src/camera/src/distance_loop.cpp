	...
	//	drawContours(frame, approx, -1, CV_RGB(0xD4, 0x35, 0xCD));
		...
		if (tool == CIRCLE && approx.size() == 8) {
			double diags[4] = {
				...
				sqrt(pow((approx[1].x - approx[5].x), 2) + pow((approx[1].y - approx[5].y), 2)) ,
				sqrt(pow((approx[2].x - approx[6].x), 2) + pow((approx[2].y - approx[6].y), 2)) ,
				sqrt(pow((approx[3].x - approx[7].x), 2) + pow((approx[3].y - approx[7].y), 2)) ,
			};

			std::cout << "CIRCLE - DIAGONALS: " << std::endl;
			std::cout << "\t0-4: " << diags[0] << std::endl;
			std::cout << "\t1-5: " << diags[1] << std::endl;
			std::cout << "\t2-6: " << diags[2] << std::endl;
			std::cout << "\t3-7: " << diags[3] << std::endl;

			double diags_avg = (diags[0] + diags[1] + diags[2] + diags[3]) / 4;
			std::cout << "CIRCLE - DIAGONALS: avg: " << diags_avg << ", 5%" << diags_avg * .05 << std::endl;

			for (int dth = 0; dth < 4; dth++) {
				if (abs(diags[dth] - diags_avg) > (diags_avg * .05)) {
					std::cout << "CIRCLE - DIAGONALS: [" << dth
							  << "] is beyond the average." << std::endl;
				}
			}
